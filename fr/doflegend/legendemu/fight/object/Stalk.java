package fr.doflegend.legendemu.fight.object;
...
public class Stalk {
	...
	public static Traque getTraqueByOwner(Characters p)
	{
		...
		{
			synchronized(_traques){ _traques.put(p.get_GUID(), new Traque(p)); }
		}
	...
	public static void notifyToOwner(Characters p, short notifyType)
	{
		String message = "";
		if(p == null) return;
		if(notifyType == CONNEXION) message = new StringBuilder("Votre cible \"").append(p.get_name()).append("\" vient de se connecter").toString(); 
		else if(notifyType == DECONNEXION)
		{
			message = new StringBuilder("Votre cible \"").append(p.get_name()).append("\" vient de se déconnecter").toString();
		}
		else return;
		...
		{
			if(t.getTarget() != p.get_GUID()) continue;
			Characters owner = World.getPersonnage(t.getOwner());
			if(owner == null || owner.get_compte() == null || owner.get_compte().getGameThread() == null || !owner.isOnline())continue;
	...
	public static void newTraque(Characters p)
	{
		...
		{
			if(perso == null || perso.get_compte() == null || !perso.isOnline()) continue;
			if(perso.get_compte().get_gmLvl() != 0) continue;
			//Vérifs d'alignement
			...
			else diff = p.get_lvl()-perso.get_lvl();
			if(diff > Config.CONFIG_TRAQUE_DIFFERENCE) continue;
			//Vérifs d'ip
			if(perso.get_compte().get_curIP().equalsIgnoreCase(p.get_compte().get_curIP())) continue;
			//On vérifie qu'il est pas dans les interdits
			if(forbidens != null && forbidens.contains(perso.get_GUID())) continue;
		...
		SocketManager.GAME_SEND_MESSAGE(p, new StringBuilder("Vous êtes en traque de \"").append(target.get_name()).append("\", votre cible est de niveau ").append(target.get_lvl()).append(", faites .chercher pour trouver votre cible.").toString(),"00A000");
	}
