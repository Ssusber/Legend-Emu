package fr.doflegend.legendemu.fight;
...
import fr.doflegend.legendemu.common.IA;
import fr.doflegend.legendemu.common.Pathfinding;
...
import fr.doflegend.legendemu.common.IA.IAThread;
import fr.doflegend.legendemu.common.World.Couple;
import fr.doflegend.legendemu.common.World.Drop;
import fr.doflegend.legendemu.fight.object.Challenge;
...
import fr.doflegend.legendemu.fight.object.Monster.MobGrade;
import fr.doflegend.legendemu.fight.object.Monster.MobGroup;
import fr.doflegend.legendemu.fight.object.Stalk.Traque;
import fr.doflegend.legendemu.game.GameSendThread;
import fr.doflegend.legendemu.game.GameServer;
import fr.doflegend.legendemu.game.GameThread.GameAction;
...
import fr.doflegend.legendemu.object.SoulStone;
import fr.doflegend.legendemu.object.Maps.Case;
import fr.doflegend.legendemu.object.Objects.ObjTemplate;
import fr.doflegend.legendemu.spell.Spell;
import fr.doflegend.legendemu.spell.SpellEffect;
import fr.doflegend.legendemu.spell.Spell.SortStats;
...
public class Fight
{
	...
	public static class Fighter
	{
		...
		{
			if(p.get_compte() == null || p.get_compte().getGameThread() == null) return;
			for(Entry<Integer, Integer> state : _state.entrySet())
			{
			SocketManager.GAME_SEND_GA_PACKET(p.get_compte().getGameThread().get_out(),7+"", 950+"", getGUID()+"", getGUID()+","+state.getKey()+",1");
			}
		}
		...
		public void decrementStates()
		{
			...
			for(Entry<Integer,Integer> e : entries)
			{
				...
				{
					//on envoie au org.walaka.rubrumsolem.client la desactivation de l'état
	...
	/** TODO: Timers by Return/Skryn - Noraj ;) **/
    		 ...
    		 Config.fightTimers.schedule(actTimerTask = new TimerTask(){
			     public void run(){
			    	 try {
			    		 ...
			    		 if (!isTurn)
			    			 GameServer.addToLog(">Début de combat échoué sur la map "+get_map().get_id()+" !");
			    		 else
			    			 GameServer.addToLog(">Fin de tour échoué ou déjà passé sur la map "+get_map().get_id()+" !");
			    		 this.cancel();
			    		 return;
			    	 }
			     }
	...
	public Fight(int type, int id,Maps map, Characters init1, Characters init2, boolean init2Protected)
	{
		...
		if(init2Protected) {
			protector = new Fighter(this, World.getMonstre(394).getGradeByLevel(Constant.getProtectorLevelByAttacker(_init0)));
		...
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init1.get_GUID()+"", init1.get_GUID()+","+Constant.ETAT_PORTE+",0");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init1.get_GUID()+"", init1.get_GUID()+","+Constant.ETAT_PORTEUR+",0");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init2.get_GUID()+"", init2.get_GUID()+","+Constant.ETAT_PORTE+",0");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init2.get_GUID()+"", init2.get_GUID()+","+Constant.ETAT_PORTEUR+",0");
		...
		if(init2Protected) {
			...
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(Fight.this, 3, 950, protector.getGUID()+"", protector.getGUID()+","+Constant.ETAT_PORTE+",0");
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(Fight.this, 3, 950, protector.getGUID()+"", protector.getGUID()+","+Constant.ETAT_PORTEUR+",0");
	...
	{	
		...
		for(Characters z : World.getGuild(_guildID).getMembers())
		{
			...
			if(z.isOnline())
			{
				...
				SocketManager.GAME_SEND_MESSAGE(z, "Un de vos percepteurs a ete attaque.", Config.CONFIG_MOTD_COLOR);
			}
		}
	}
	...
	public Fight(int id, Maps map, ArrayList<Characters> team1, ArrayList<Characters> team2) {//TODO KOLI
		...
		_st1 = 0;
		_st2 = 1;
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, team1.get(0).get_GUID()+"", team1.get(0).get_GUID()+","+Constant.ETAT_PORTE+",0");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, team1.get(0).get_GUID()+"", team1.get(0).get_GUID()+","+Constant.ETAT_PORTEUR+",0");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, team2.get(0).get_GUID()+"", team2.get(0).get_GUID()+","+Constant.ETAT_PORTE+",0");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, team2.get(0).get_GUID()+"", team2.get(0).get_GUID()+","+Constant.ETAT_PORTEUR+",0");
		_init0.set_fightCell(getRandomCell(_start0));
		_init1.set_fightCell(getRandomCell(_start1));
		_init0.getPersonnage().get_curCell().removePlayer(_init0.getGUID());
		_init1.getPersonnage().get_curCell().removePlayer(_init1.getGUID());
	...
	public void verifIfAllReady()
	{
		...
		if(_type != Constant.FIGHT_TYPE_PVM && _type != Constant.FIGHT_TYPE_PVT && _type != Constant.FIGHT_TYPE_CONQUETE && protector == null)
		{
	...
	private void startFight()
	{
		...
		if(_type == Constant.FIGHT_TYPE_PVT)
		{
			...
			for(Characters z : World.getGuild(_guildID).getMembers())
			{
				...
				if(z.isOnline())
				{
					...
					Collector.parseAttaque(z, _guildID);
					Collector.parseDefense(z, _guildID);
					SocketManager.GAME_SEND_MESSAGE(z, "Un de vos percepteurs est rentre en combat.", Config.CONFIG_MOTD_COLOR);
				}
			}
		}
		...
		{
			if(f == null || f.getPersonnage() == null) continue;
			f.setSpellStats();
		...
		if(getActTimerTask()  != null)getActTimerTask().cancel();
		...
		for(Fighter F : getFighters(3))
		{
			...
			if(perso.isOnMount())
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID()+"", perso.get_GUID()+","+Constant.ETAT_CHEVAUCHANT+",1");
		...
		if(get_type() == Constant.FIGHT_TYPE_PVM)
		{
			...
			if(!getMobGroup().isFix() && isCheckTimer())
				World.getCarte(get_map().get_id()).spawnAfterTimeGroup(-1, 1, true, -1);//Respawn d'un groupe	
			if(getMobGroup().isFix() && isCheckTimer() && get_map().get_id() != 6826)
				World.getCarte(get_map().get_id()).spawnAfterTimeGroupFix(getMobGroup().getCellID());//Respawn d'un groupe	
		}
		startTurn();
	...
	private void startTurn()
	{
		...
		if(Thread.interrupted())
		{
			...
			catch(InterruptedException ie)
			{
				...
				for(Fighter f : fighters)
				{
					...
					if(p == null || p.get_compte() == null || p.get_compte().getGameThread()==null || p.get_compte().getGameThread().get_out() == null) continue;
					havePlayers = true;
					break;
				}
				if(!havePlayers)
		...
		if(_ordreJeu.get(_curPlayer).getPersonnage() == null 
				...
				|| _ordreJeu.get(_curPlayer)._Perco != null || (((Fighter)this._ordreJeu.get(this._curPlayer))._Prisme != null))//Si ce n'est pas un joueur
		{
	...
	public synchronized void endTurn(boolean isAuto)
	{
		...
		try
		{
			...
			if(!isAuto && !_curAction.equals("") && _ordreJeu.get(_curPlayer).getPersonnage() != null){//On enlève la boucle si c'est un action effectuée à la dernière seconde - Skryn/Return
			...
			_curAction = "";
			if (isAuto){ //Puis on laisse le temps de terminer l'action si on a une lancée à la dernière seconde |Skryn/Return
				try {
					Thread.sleep(2100);
	...
	public synchronized boolean fighterDeplace(Fighter f, GameAction GA)
	{
		...
		if( nStep > _curFighterPM || nStep == -1000)
		{
			...
			if(f.getPersonnage() != null && f.getPersonnage().get_compte() != null && f.getPersonnage().get_compte().getGameThread() != null){
				SocketManager.GAME_SEND_GA_PACKET(f.getPersonnage().get_compte().getGameThread().get_out(), ""+151, ""+f.getGUID(), "-1", "");
			}
			return false;
		}
	...
	{
		if(_curAction.equals("")|| _ordreJeu.get(_curPlayer).getGUID() != perso.get_GUID() || _state!= Constant.FIGHT_STATE_ACTIVE)return;
		if(Config.debug) GameServer.addToLog("("+_curPlayer+")Fin du deplacement de Fighter ID= "+perso.get_GUID());
		SocketManager.GAME_SEND_GAMEACTION_TO_FIGHT(this,7,_curAction);
	...
	public synchronized int tryCastSpell(Fighter fighter,SortStats Spell, int caseID)
	{
		...
		if(CanCastSpell(fighter,Spell,Cell, -1))
		{
			...
			// le org.walaka.rubrumsolem.client ne peut continuer sans l'envoi de ce packet qui annonce le coût en PA
	  ...
	  public boolean CanCastSpell2(Fighter fighter, Spell.SortStats spell, Maps.Case cell, int launchCase)
	  {
	    ...
	    {
	      if (Config.debug) GameServer.addToLog("Sort non existant");
	      return false;
	    }
	    if (_ordreJeu.get(_curPlayer) == null) return false;
	    if (((Fighter)_ordreJeu.get(_curPlayer)).getGUID() != fighter.getGUID())// Si ce n'est pas sont tour
	    {
	      if (Config.debug) GameServer.addToLog("Ce n'est pas au joueur de jouer");
	      return false;
	    }
	    if (_curFighterPA < spell.getPACost(fighter))// Si il na pas asser de pa
	    {
	      if (Config.debug) GameServer.addToLog("Le joueur n'a pas assez de PA (" + _curFighterPA + "/" + spell.getPACost(fighter) + ")");
	      return false;
	    }
	    if (cell == null)// Si cellID inexistante
	    {
	      if (Config.debug) GameServer.addToLog("La cellule visee n'existe pas");
	      return false;
	    }
	    if ((spell.isLineLaunch(fighter)) && (!Pathfinding.casesAreInSameLine(_map, launchCase, cell.getID(), 'z')))// Si ont lance le sort la ou ont peut pas
	    {
	      if (Config.debug) GameServer.addToLog("Le sort demande un lancer en ligne, or la case n'est pas alignee avec le joueur");
	      return false;
	    }
	    if ((spell.hasLDV(fighter)) && (!Pathfinding.checkLoS(_map, launchCase, cell.getID(), fighter)))// Si problÂŽme de cellID
	    {
	      if (Config.debug) GameServer.addToLog("Le sort demande une ligne de vue, mais la case visee n'est pas visible pour le joueur (IA)");
	      return false;
	    }
	    ...
	    {
	      if (Config.debug) GameServer.addToLog("La case est trop proche ou trop eloignee(2) (IA) Min: " + spell.getMinPO() + " Max: " + spell.getMaxPO(fighter) + " Dist: " + dist);
	      return false;
	    }
...
/* 1749 */     if (spell == null)
/*      */     {
/* 1751 */       if (Config.debug) GameServer.addToLog("(" + this._curPlayer + ") Sort non existant");
/* 1752 */       if (perso != null)
/*      */       {
...
/*      */     {
/* 1761 */       if (Config.debug) GameServer.addToLog("Ce n'est pas au joueur. Doit jouer :(" + f.getGUID() + "). Fautif :(" + fighter.getGUID() + ")");
/* 1762 */       if (perso != null)
/*      */       {
...
/*      */     {
/* 1771 */       if (Config.debug) GameServer.addToLog("(" + this._curPlayer + ") Le joueur n'a pas assez de PA (" + this._curFighterPA + "/" + spell.getPACost(fighter) + ")");
/* 1772 */       if (perso != null)
/*      */       {
/* 1774 */         SocketManager.GAME_SEND_Im_PACKET(perso, "1170;" + this._curFighterPA + "~" + spell.getPACost(fighter));
/*      */       }
/* 1776 */       return false;
/*      */     }
/*      */
/* 1779 */     if (cell == null)
/*      */     {
/* 1781 */       if (Config.debug) GameServer.addToLog("(" + this._curPlayer + ") La cellule visee n'existe pas");
/* 1782 */       if (perso != null)
/*      */       {
...
/*      */     }
/*      */
/* 1789 */     if ((spell.isLineLaunch(fighter)) && (!Pathfinding.casesAreInSameLine(this._map, ValidlaunchCase, cell.getID(), 'z')))
/*      */     {
/* 1791 */       if (Config.debug) GameServer.addToLog("(" + this._curPlayer + ") Le sort demande un lancer en ligne, or la case n'est pas alignee avec le joueur");
/* 1792 */       if (perso != null)
/*      */       {
...
/*      */     }
/*      */
/* 1799 */     if ((spell.hasLDV(fighter)) && (!Pathfinding.checkLoS(this._map, ValidlaunchCase, cell.getID(), fighter)))
/*      */     {
/* 1801 */       if (Config.debug) GameServer.addToLog("(" + this._curPlayer + ") Le sort demande une ligne de vue, mais la case visee n'est pas visible pour le joueur");
/* 1802 */       if (perso != null)
/*      */       {
...
/*      */     {
/* 1818 */       if (Config.debug) GameServer.addToLog("(" + this._curPlayer + ") La case est trop proche ou trop eloignee Min: " + spell.getMinPO() + " Max: " + spell.getMaxPO(fighter) + " Dist: " + dist);
/* 1819 */       if (perso != null)
/*      */       {
/* 1821 */         SocketManager.GAME_SEND_Im_PACKET(perso, "1171;" + spell.getMinPO() + "~" + spell.getMaxPO(fighter) + "~" + dist);
/*      */       }
/* 1823 */       return false;
/*      */     }
/*      */
...
/*      */   }
			...
			if (s.getEffectID() == 185 || s.getEffectID() == 181 || s.getEffectID() == 180 || s.getEffectID() == 400 || s.getEffectID() == 780){
				for (Fighter p: getAllFighters()){
					if (cell == p.get_fightCell()){
						SocketManager.GAME_SEND_MESSAGE(perso, "La case visée n'est pas disponible.", Config.CONFIG_MOTD_COLOR);
						return false;
					}
				}
			}
	...
	/**	if (isMob){
		...
		if (spell.getSpellID() == 435){
			if (fighter.hasBuff(9)){
				SocketManager.GAME_SEND_MESSAGE(perso, "Le sort Transfert de Vie est innutilisable lorsque vous êtes sous l'emprise du sort Dérobade.", Config.CONFIG_MOTD_COLOR);
				return false;
			}
		}**/
		...
		if(dist < spell.getMinPO() || dist > MaxPO)
		{
			...
			if(perso != null)
			{
				SocketManager.GAME_SEND_Im_PACKET(perso, "1171;" + spell.getMinPO() + "~" + spell.getMaxPO(fighter) + "~" + dist);
			}
			return false;
		}
		...
		{
			SocketManager.GAME_SEND_MESSAGE(perso, "Il est impossible de lancer coopération et transposition dans un même tour de jeu !", Config.CONFIG_MOTD_COLOR);
			return false;
		}
		return true;
	}
	...
	public synchronized String GetGE(int win)
    {
       ...
       // int TEAM2lvl = 0;
        ...
        for(Fighter F : TEAM2)
        {
        	...
        	{
	        	if(traque.getTarget() == F.getPersonnage().get_GUID())
	        	{ 
	        		SocketManager.GAME_SEND_MESSAGE(curp, "Vous venez de tuer le personnage que vous traquiez, faites .recompense pour obtenir votre récompense.", "0000A0");
	        		traque.valider();
	        	}
        	}
      ...
      //Challenge augmente la PP totale (atteint plus facilement les seuils)
		...
		if ((this._type == 4) && (this._challenges.size() > 0)) {
  	     ...
  	     factChalDrop += _mobGroup.get_bonusValue(); // on ajoute le bonus en étoiles
		}
 		factChalDrop /= 100;
	    ...
	    //Fin Capture
	        /* Testons Configtenant de mettre l'apprivoisement des DD
	        ...
	        if(_apprivoiseur.size() > 0) // Si il y a des captureurs
	        {
	        	...
	        	if(!(f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME).getTemplate().getType() == Constant.ITEM_TYPE_FILET_CAPTURE))// Qui possèdent un filet
				{
	    ...
	    for(Fighter i : TEAM1)
		{
        	...
        	if(type == Constant.FIGHT_TYPE_CHALLENGE)
        	{
        		...
        		if (i.getPersonnage() != null && i.getPersonnage().getKolizeum() == 1)
        		{
					...
					SocketManager.GAME_SEND_MESSAGE(i.getPersonnage(), "Fécilitation ! Vous remportez ce match de kolizeum."+
																		"\nVous gagnez 5 000 000 d'expérience ainsi que 2 Kolizetons et 10 000 000 de kamas.",Config.CONFIG_MOTD_COLOR);
					GameServer.addToLog("Le personnage "+i.getPersonnage()+" a gagné son match de kolizeum et reçoit les récompenses");
        	...
        	}else
        	{
        		...
        		if(type == Constant.FIGHT_TYPE_AGRESSION)
        		{
					...
					{
						if(_init1.getPersonnage().get_compte().get_curIP().compareTo(_init0.getPersonnage().get_compte().get_curIP()) != 0 || Config.ALLOW_MULE_PVP)
						{
					...
					if (P != null) {
		        		...
		        		P.addHonor(winH);
		        		P.setDeshonor(P.getDeshonor()+winD);
		        		SocketManager.GAME_SEND_MESSAGE(P, "Vous venez de remporter la victoire ! Vous gagnez "+winH+" points d'honneur !", Config.CONFIG_MOTD_COLOR);
		        		Packet.append("2;").append(i.getGUID()).append(";").append(i.getPacketsName()).append(";").append(i.get_lvl()).append(";").append((i.isDead() ?  "1" : "0" )).append(";");
		        		Packet.append((P.get_align()!=Constant.ALIGNEMENT_NEUTRE?World.getExpLevel(P.getGrade()).pvp:0)).append(";");
		        		Packet.append(P.get_honor()).append(";");
		        		int maxHonor = World.getExpLevel(P.getGrade()+1).pvp;
		...
		for(Fighter i : TEAM2)
		{
			if (i.getPersonnage() != null && i.getPersonnage().getKolizeum() == 1)
			{
				SocketManager.GAME_SEND_MESSAGE(i.getPersonnage(), "Dommage, vous avez perdu ! Mais retentez vite votre chance ...", Config.CONFIG_MOTD_COLOR);
				i.getPersonnage().setKolizeum(-1);
				i.getPersonnage().teleport(i.getPersonnage().getLastMapFight(), World.getCarte(i.getPersonnage().getLastMapFight()).getRandomFreeCellID());
			}
		...
		if ((win != 1) && (_type == 4))
		{
		_mapOld.spawnGroupOnDead(this._mobGroup.getAlignement(), 1, true, this._mobGroup.getCellID(), this._mobGroup);
		}
	...
	public void verifIfTeamAllDead()
	{
		...
		if(team0 || team1 || !verifyStillInFight())
		{
			...
			for(Entry<Integer, Fighter> entry : _team0.entrySet())//Team joueurs
			{
				...
				perso.set_ready(false);
				perso.set_fight(null);
				GameServer.addToLog("Le Personnage "+perso.get_name()+" vient de terminer un combat");
			}
			...
			for(Fighter F : winTeam)
			{
				...
				if(F._Perco != null)
				{
					...
					{
						if(z == null) continue;
						if(z.get_compte() == null || z.get_compte().getGameThread() == null) continue;
	        	...
	        	if(_type == Constant.FIGHT_TYPE_CHALLENGE)
	        		if(F.getPersonnage().get_curCarte().getSubArea().get_subscribe() && F.getPersonnage().get_compte().get_subscriber() == 0 && Config.USE_SUBSCRIBE)//Non abonné
	        		{
	        				//On le place a sa statue
	        				F.getPersonnage().teleport(Constant.getClassStatueMap(F.getPersonnage().get_classe()), Constant.getClassStatueCell(F.getPersonnage().get_classe()));
	        	}
				if(_type != Constant.FIGHT_TYPE_CHALLENGE) 
			...
			for(Fighter F : looseTeam)
			{
				...
				if(F._Perco != null)
				{
					...
					for(Characters z : World.getGuild(_guildID).getMembers())
					{
						...
						{
							SocketManager.GAME_SEND_gITM_PACKET(z, Collector.parsetoGuild(z.get_guild().get_id()));
							SocketManager.GAME_SEND_MESSAGE(z, "Votre percepteur est mort.", Config.CONFIG_MOTD_COLOR);
						}
					}
				}
				if (F._Prisme != null) {
					fr.doflegend.legendemu.common.World.SubArea subarea = _mapOld.getSubArea();
					for (Characters z : World.getOnlinePersos()) {
						if (z == null)
							continue;
						if (z.get_align() == 0) {
	...
	public synchronized void tryCaC(Characters perso, int cellID)
	{
		...
		}else
		{
			...
			if(!Pathfinding.canUseCaConPO(this, _ordreJeu.get(_curPlayer).get_fightCell().getID(), cellID, arme))
		   {
			...
			if(isEc)
			{
				if(Config.debug) GameServer.addToLog(perso.get_name()+" Echec critique sur le CaC ");
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 305, perso.get_GUID()+"", "");//Echec Critique Cac
				SocketManager.GAME_SEND_GAF_PACKET_TO_FIGHT(this, 7, 0, perso.get_GUID());//Fin de l'action
				endTurn(false);
			}else
			{
				...
				if(isCC)
				{
					if(Config.debug) GameServer.addToLog(perso.get_name()+" Coup critique sur le CaC");
				...
				{					
					...
					/**if (caster.hasBuff(9)){
						if (SE.getEffectID() == 90){
							SocketManager.GAME_SEND_MESSAGE(caster.getPersonnage(), "Sort innutilisable quand dérobage est actif !", Config.CONFIG_MOTD_COLOR);
						}
					}**/
	...
	public boolean reconnexion(Characters perso)
	{
		...
		if(_state == Constant.FIGHT_STATE_ACTIVE) SocketManager.GAME_SEND_GJK_PACKET(perso,_state,0,0,0,0,_type);//Join Fight => _state, pas d'anulation...
		else
		{
			if(_type == Constant.FIGHT_TYPE_CHALLENGE)
			{
	...
	public boolean deconnexion(Characters perso, boolean verif)//True si entre en mode déconnexion en combat, false sinon
	{
		Fighter f = getFighterByPerso(perso);
		if(f == null) return false;
		...
		if(f.getNBDeco() >= 5)
		{
			if (!verif){
				...
				for(Fighter e : this.getFighters(7))
				{
					...
					SocketManager.GAME_SEND_MESSAGE(e.getPersonnage(), f.getPacketsName()+ " s'est déconnecté plus de 5 fois dans le même combat, nous avons décidé de lui faire abandonner.", "A00000");
				}
			}
			return false;
		}
				...
				case Constant.FIGHT_TYPE_CONQUETE://Prismes
					...
					}else if(_state == Constant.FIGHT_STATE_PLACE)
					{
						...
						if(isValid1)//Celui qui fait l'action a lancer le combat et leave un autre personnage
						{
							...
							{
								if(Config.debug) System.out.println("EXLUSION DE : "+T.getPersonnage().get_name());
								SocketManager.GAME_SEND_ON_FIGHTER_KICK(this, T.getPersonnage().get_GUID(), getTeamID(T.getGUID()));
								if(_type == Constant.FIGHT_TYPE_AGRESSION || _type == Constant.FIGHT_TYPE_CHALLENGE || _type == Constant.FIGHT_TYPE_PVT) SocketManager.GAME_SEND_ON_FIGHTER_KICK(this, T.getPersonnage().get_GUID(), getOtherTeamID(T.getGUID()));
								...
								for(Characters z : _mapOld.getPersos()) FightStateAddFlag(this._mapOld, z);
							}
						}else if(T == null)//Il leave de son plein gré donc (T = null)
						{
							...
							if(isValid2)//Soit il a lancer le combat => annulation du combat
							{
								...
								if(_type == Constant.FIGHT_TYPE_PVT)
								{
									...
									for(Characters z : World.getGuild(_guildID).getMembers())
									{
										...
										{
											SocketManager.GAME_SEND_gITM_PACKET(z, Collector.parsetoGuild(z.get_guild().get_id()));
											SocketManager.GAME_SEND_MESSAGE(z, "Votre percepteur remporte la victioire.", Config.CONFIG_MOTD_COLOR);
										}
									}
							...
							}else//Soit il a rejoin le combat => Left de lui seul
							{
								...
								if(_type == Constant.FIGHT_TYPE_AGRESSION || _type == Constant.FIGHT_TYPE_CONQUETE|| _type == Constant.FIGHT_TYPE_CHALLENGE || _type == Constant.FIGHT_TYPE_PVT) SocketManager.GAME_SEND_ON_FIGHTER_KICK(this, F.getPersonnage().get_GUID(), getOtherTeamID(F.getGUID()));
								Characters P = F.getPersonnage();
								P.set_duelID(-1);
								P.set_ready(false);
								P.fullPDV();
	...
	public void joinAsSpect(Characters p)
	{
		...
		p.set_fight(this);
		if(p.get_compte().get_gmLvl() == 0) SocketManager.GAME_SEND_Im_PACKET_TO_FIGHT(this, 7, "036;"+p.get_name());
		...
		for(Entry<Integer, Fight> fight : _map.get_fights().entrySet())
		{
			if(fight.getValue()._state == Constant.FIGHT_STATE_PLACE)
			{
				if(fight.getValue()._type == Constant.FIGHT_TYPE_CHALLENGE)
				{
					...
					{
						if(Config.debug) System.out.println(F.getValue().getPersonnage().get_name());
				...
				}else if(fight.getValue()._type == Constant.FIGHT_TYPE_PVM)
				{
					...
					for(Entry<Integer, Fighter> F : fight.getValue()._team0.entrySet())
					{
						if(Config.debug) System.out.println("PVM1: "+F.getValue().getPersonnage().get_name());
						SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_PLAYER(P, fight.getValue()._init0.getPersonnage().get_curCarte(),fight.getValue()._init0.getGUID(), fight.getValue()._init0);
					}
					for(Entry<Integer, Fighter> F : fight.getValue()._team1.entrySet())
					{
						if(Config.debug) System.out.println("PVM2: "+F.getValue());
				...
				}else if(fight.getValue()._type == Constant.FIGHT_TYPE_PVT)
				{
					...
					for(Entry<Integer, Fighter> F : fight.getValue()._team0.entrySet())
					{
						if(Config.debug) System.out.println("PVT1: "+F.getValue().getPersonnage().get_name());
						SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_PLAYER(P, fight.getValue()._init0.getPersonnage().get_curCarte(),fight.getValue()._init0.getGUID(), fight.getValue()._init0);
					}
					for(Entry<Integer, Fighter> F : fight.getValue()._team1.entrySet())
					{
						if(Config.debug) System.out.println("PVT2: "+F.getValue());
					...
					SocketManager.GAME_SEND_GAME_ADDFLAG_PACKET_TO_PLAYER(P, fight.getValue()._init0.getPersonnage().get_curCarte(),
							0, fight.getValue()._init0.getGUID(), fight.getValue()._Prisme.getID(), fight.getValue()._init0
