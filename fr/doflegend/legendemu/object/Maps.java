...
import fr.doflegend.legendemu.common.*;
...
import fr.doflegend.legendemu.game.*;
...
public class Maps {
	...
	public static class MountPark
	{
		...
		{
			try {
				for(Entry<Short, fr.doflegend.legendemu.object.Maps.MountPark> mp : World.getMountPark().entrySet())//Pour chaque enclo si ils en ont plusieurs
				{
	...
	{
		private int _id;
		private Map<Integer, Characters>	_persos;		//= new TreeMap<Integer, Personnage>();
	    ...
	    public synchronized void activeCell()
	    {
	      ...
	      this._timer = new Timer(30000, new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	          ...
	          try {
	            Thread.sleep(1500L); } catch (Exception localException) {
	          }
		...
		{
			if(_persos == null) _persos = new TreeMap<Integer, Characters>();
		...
		{
			if(_persos == null) return new TreeMap<Integer, Characters>();
		...
		public void startAction(Characters perso, GameAction GA)
		{
			...
			switch(actionID)
			{
				...
				case 157: //Zaapis
					...
					if (perso.get_curCarte()._subArea.getArea().getID() == 7 && (perso.get_align() == 1 || perso.get_align() == 0 || perso.get_align() == 3))//Ange, Neutre ou Sérianne
					{
						Zaapis = Constant.ZAAPI.get(Constant.ALIGNEMENT_BONTARIEN).split(",");
						if (perso.get_align() == 1) price = 10;
					}
					else if (perso.get_curCarte()._subArea.getArea().getID() == 11 && (perso.get_align() == 2 || perso.get_align() == 0 || perso.get_align() == 3))//Démons, Neutre ou Sérianne
					{
	...
	public void spawnAfterTimeGroup(int i, int j, boolean b, int k) 
	{
		...
		respawnTimer = new Timer(20000, new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				respawnTimer.stop();
				World.getCarte((short) map).spawnGroup(-1, nbrFinal, true, -1);//Respawn d'un groupe
				respawnTimer.stop();
			}
		});
		respawnTimer.start();
	...
	public void spawnGroup(int align, int nbr,boolean log,int cellID)
	{
		...
		for(int a = 1; a<=nbr;a++)
		{
			...
			if(log) {
				GameServer.addToLog("- Un groupe de monstre viens d'être ajouté sur la Map ("+this._id+") d'alignement "+align+" avec comme ID : "+this._nextObjectID+" !");
	...
	public void spawnNewGroup(boolean timer,int cellID,String groupData,String condition)
	{
		...
		if (log)
		{
		GameServer.addToLog("Groupe de monstres ajoutes sur la map: " + this._id + " alignement: " + align + " ID: " + this.get_nextObjectID());
		SocketManager.GAME_SEND_MAP_MOBS_GM_PACKET(this, group);
		}
	...
	public void onPlayerArriveOnCell(Characters perso,int caseID)
	{
		...
		for(MobGroup group : _mobGroups.values())
		{
			if(Pathfinding.getDistanceBetween(this,caseID,group.getCellID()) <= group.getAggroDistance())//S'il y aggro
			{
				...
				{
					if(perso.get_compte().get_subscriber() == 0 && getSubArea().get_subscribe() && Config.USE_SUBSCRIBE) return;
	...
	public void startFigthVersusMonstres(Characters perso, MobGroup group) //Vérifier à chaque combat l'ID et voir si c'est le même. on verra donc si ya colision
	{
		int id = 1;
		if(!_fights.isEmpty())
			id = ((Integer)(_fights.keySet().toArray()[_fights.size()-1]))+1;
	...
	public void iniciarFightVSPrisme(Characters perso, Prism Prisme) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer)(_fights.keySet().toArray()[_fights.size()-1]))+1;
