...
public class Mount {
	...
	public void addObjToSac(int id, int qua, Characters perso) {
		...
		{
			GameServer.addToLog("Le joueur "+perso.get_name()+" a tenter d'ajouter un objet au store qu'il n'avait pas.");
			return;
		}
		if (perso.getItems().get(objToAdd.getGuid()).getQuantity() < qua)
		{
			GameServer.addToLog("Le joueur "+perso.get_name()+" a tenté d'ajouter une quantité d'objet en banque dont il ne possédait pas.");
