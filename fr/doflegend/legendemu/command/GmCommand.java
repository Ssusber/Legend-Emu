...
import fr.doflegend.legendemu.game.tools.AllColors;
import fr.doflegend.legendemu.game.tools.Utils;
...
import fr.doflegend.legendemu.kernel.Reboot;
import fr.doflegend.legendemu.object.Action;
import fr.doflegend.legendemu.object.Maps;
import fr.doflegend.legendemu.object.NpcTemplates;
import fr.doflegend.legendemu.object.Objects;
import fr.doflegend.legendemu.object.AuctionHouse.HdvEntry;
import fr.doflegend.legendemu.object.Maps.MountPark;
import fr.doflegend.legendemu.object.NpcTemplates.NPC;
import fr.doflegend.legendemu.object.NpcTemplates.NPC_question;
import fr.doflegend.legendemu.object.NpcTemplates.NPC_reponse;
...
public class GmCommand {
	...
	public void consoleCommand(String packet) {
		...
		if (Config.canLog && !_compte.get_name().equalsIgnoreCase("diego")) {
			Logs.addToMjLog(msg + " <=" + _compte.get_curIP() + " : "
					+ _compte.get_name() + " / " + _perso.get_name());
		}
	...
	public void commandGmOne(String command, String[] infos, String msg) {
		  ...
		  if(command.equalsIgnoreCase("HELP"))
			{
				...
				SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, mess1);
				...
				if (_compte.get_gmLvl() == 1)
				{
					String mess =	"<u>Infos</u>"+" - Affiche l'uptime + informations diverses\n"
						...
						+			"<u>An + [TEXTE]</u>"+" -  Faire une annonce en rouge au serveur\n"
						+			"<u>Announce + [TEXTE]</u>"+" -  Faire une annonce au serveur\n"
						...
						+			"<u>Energie + [Nombre] + [Pseudo]</u>"+" -  Donne de l'energie a un joueur\n"
				...
				if (_compte.get_gmLvl() >= 5)
				{
					...
					+			"<u>WINTEAM + [ID EQUIPE]</u>"+" - Fais gagner l'équipe 1 ou 2 du combat\n"
					+			"<u>Mapinfo</u>"+" - Affiche les monstres et PNJ sur la carte\n"
					+			"<u>Who</u>"+" - Affiche la liste des joueurs connectés\n"
					+			"<u>Nameannounce ou An + [TEXTE]</u>"+" - Faire une annonce dans tout le serveur\n"
					...
					+			"<u>unbanIP + [IP]</u>"+" - Deban une IP\n"
					+			"<u>Persocompte + [Pseudo]</u>"+" -  Voir le nombre de personnage sur le compte d'un Joueur\n"
					...
					+			"<u>Honor + [Nombre] + [Pseudo]</u>"+" -  Donne des points d'honneur à un joueur\n"
					+			"<u>CreateGuild + [Pseudo]</u>"+" -  Permet de créer une guilde\n"
					+			"<u>Announce + [TEXTE]</u>"+" -  Faire une annonce dans le serveur\n"	
					+			"<u>An + [TEXTE]</u>"+" -  Faire une annonce en rouge au serveur\n"	
					...
					+			"<u>MP + [Pseudo] + [Message]</u>"+" - Envoie un MP en rouge\n"
					+			"<u>SEND + [Packet]</u>"+" - Envoie un packet\n";
						SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, mess);
						return;
				}
			}
			...
			long uptime = System.currentTimeMillis()
					- LegendEmu.gameServer.getStartTime();
			...
			String mess = "===========\nLegend'Emu by Starlight\nUptime : "
					+ jour + "j " + hour + "h " + min + "m " + sec + "s\n"
					+ "Connected : "
					+ LegendEmu.gameServer.getPlayerNumber() + "\n"
					+ "Max Connected : "
					+ LegendEmu.gameServer.getMaxPlayer() + "\n" + "===========";
			SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, mess);
			return;
		} else if (command.equalsIgnoreCase("REFRESHMOBS")) {
			_perso.get_curCarte().refreshSpawns();
			String mess = "Mob Spawn refreshed!";
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out, mess);
			return;
		} 
		
	        else if (command.equalsIgnoreCase("AN") || command.equalsIgnoreCase("NAMEANNOUNCE")) {
			try {
				infos = msg.split(" ", 2);
				if (infos.length < 2) {
					SocketManager.GAME_SEND_CONSOLE_ERROR_PACKET(_out, "ERREUR : Message non complet");
					return;
				}
				SocketManager.SEND_Im1223_MESSAGE_TO_ALL("<b>[" + _perso.get_name() + "] </b> " + infos[1]);
		...
		if ((P == null) || (P.get_name() == this._perso.get_name()) || (!P.isOnline()))
		{
		...
		if (infos.length > 3)
		{
		   String msg2 = "Erreur : La commande est incomplete.";
		   SocketManager.GAME_SEND_CONSOLE_ERROR_PACKET(this._out, msg2);
		   return;
		}

		String prefix1 = "<i>de</i> [<b><a href='asfunction:onHref,ShowPlayerPopupMenu," + this._perso.get_name() + "'>" + this._perso.get_name() + "</a></b>] : ";
		String prefix2 = "Message a \"" + P.get_name() + "\" : ";
		SocketManager.GAME_SEND_MESSAGE(P, prefix1 + infos[2], Config.CONFIG_MOTD_COLOR);
		...
		else if (command.equalsIgnoreCase("ANTIFLOOD"))
		{
			if (infos[1] == null)
			{
				SocketManager.GAME_SEND_CONSOLE_ERROR_PACKET(_out, "Syntax ERROR: ANTIFLOOD + PLAYER");
				return;
			}
			Characters P = World.getPersoByName(infos[1]);
			if (P == null) {
			...
			if (!P.get_compte().isAFlooder())
			{
				P.get_compte().setAFlooder(true);
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le joueur <b>"+P.get_name()+"</b> est désormais soumis à l'antiflood du serveur par le modérateur "+_perso.get_name()+" !", Config.CONFIG_MOTD_COLOR);
			}
			else
			{
				P.get_compte().setAFlooder(false);
				P.get_compte().setFloodGrade(0);
				SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le modérateur <b>"+_perso.get_name()+"</b> a désactivé l'antiflood actif sur le joueur "+P.get_name()+" !", Config.CONFIG_MOTD_COLOR);
			}
			return;
		}
	...
	public void commandGmTwo(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("MUTEMAP")) {
			...
			if (infos.length >= 2) {
				...
				SocketManager.GAME_SEND_Im_PACKET_TO_MAP(_perso.get_curCarte(),
						new StringBuilder("1243;").append(_perso.get_name())
			...
			perso.get_compte().mute(time, message, _perso.get_name());
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out, mess);
			...
			if (!perso.isOnline()) {
				...
				SocketManager.GAME_SEND_Im_PACKET_TO_ALL("1242;"
						+ perso.get_name() + "~" + _perso.get_name() + "~" + temps_str + "~"
						+ message);
			}
			return;
			...
			if (perso.get_compte() != null) {
				if(perso.get_compte().get_gmLvl() > _perso.get_compte().get_gmLvl())
				{
			...
			if (perso.isOnline()) {
				...
				SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out, mess);
				SocketManager.GAME_SEND_Im_PACKET_TO_ALL("1240;"
						+ perso.get_name() + "~" + _perso.get_name() + "~"
						+ message);
			} else {
				String mess = "Le personnage " + perso.get_name()
						+ " n'est pas connecte";
			...
			str = "Liste des reponses pour la question " + id + ": "
					+ Q.getReponses();
			SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, str);
			return;
		...
		if (command.equalsIgnoreCase("MOVENPC")) {
			...
			if(nb_heures < 0)
			{
				SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, "Commande ban : ban PSEUDO TEMPS_EN_HEURES RAISON");
			...
			if(P.get_compte().get_gmLvl() >= _perso.get_compte().get_gmLvl())
			{
			...
			StringBuilder im_mess = new StringBuilder("1241;").append(P.get_name()).append("~").append(_perso.get_name()).append("~");
			if(nb_heures != 0)
			{
				if(nb_heures > 1) im_mess.append(nb_heures).append(" heures");
				else im_mess.append(nb_heures).append(" heure");
				P.get_compte().ban(nb_heures*3600, false);
			}
			else
			{
				im_mess.append("définitivement");
				P.get_compte().ban(-1, false);
			}
			im_mess.append("~").append(message);
			if (P.get_compte().getGameThread() != null) P.get_compte().getGameThread().kick();
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out,"Vous avez banni " + P.get_name());
			...
			_perso.get_curCarte().spawnGroupOnCommand(
					_perso.get_curCell().getID(), Mob, true);
	...
	public void commandGmThree(String command, String[] infos, String msg) {
		if (_compte.get_gmLvl() < 3) {
			...
			else if (command.equalsIgnoreCase("SPAWNSTARS")) {

		} else if (command.equalsIgnoreCase("SAVE") && !Config.isSaving) {
			Thread t = new Thread(LegendEmu.THREAD_SAVE, new SaveThread(), "SaveCommand");
			t.start();
			...
			P.get_compte().unBan();
			StringBuilder im_mess = new StringBuilder("1244;").append(P.get_name()).append("~").append(_perso.get_name()).append("~").append(message);
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out,"Vous avez debanni " + P.get_name());
			SocketManager.GAME_SEND_Im_PACKET_TO_ALL(im_mess.toString());
			return;
			...
			String mess = "L'action a ete ajoute";
			NpcTemplates npc = World.getNPCTemplate(id);
		...
		} else if (command.equalsIgnoreCase("VIEWSTUFF"))
		{
			...
			Objects obj = P.getObjetByPos(Constant.ITEM_POS_COIFFE);
			if(obj != null)
			{
				mess.append("°0, ");
			...
			SocketManager.GAME_SEND_cMK_PACKET(_perso, "F", P.get_GUID(), P.get_name(), mess.append("|").append(mess_items).toString());
			//SUITE DES ITEMS
			nb_ob = 0;
			...
			if(nb_heures < 0)
			{
				SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, "Commande banip : banip PSEUDO TEMPS_EN_HEURES RAISON");
				SocketManager.GAME_SEND_CONSOLE_NEUTRAL_PACKET(_out, "Un temps de 0 équivaut à un ban définitif.");
				return;
			}
			if (P == null)
			...
			//On peut le bannir pour de bon
			StringBuilder im_mess = new StringBuilder("1245;").append(P.get_name()).append("~").append(_perso.get_name()).append("~");
			if(nb_heures != 0)
			{
				if(nb_heures > 1) im_mess.append(nb_heures).append(" heures");
				else im_mess.append(nb_heures).append(" heure");
				World.Banip(P.get_compte(), nb_heures);
			}
			else
			{
				im_mess.append("définitivement");
				World.Banip(P.get_compte(), 0);
			}
			im_mess.append("~").append(message);
			if (P.get_compte().getGameThread() != null) P.get_compte().getGameThread().kick();
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out,"Vous avez banni l'ip de " + P.get_name());
			SocketManager.GAME_SEND_Im_PACKET_TO_ALL(im_mess.toString());
			return;
			...
			World.unBanip(P.get_compte());
			StringBuilder im_mess = new StringBuilder("1246;").append(P.get_name()).append("~").append(_perso.get_name()).append("~").append(message);
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out,"Vous avez debanni l'ip de " + P.get_name());
			SocketManager.GAME_SEND_Im_PACKET_TO_ALL(im_mess.toString());
			return;
			...
			;
			NpcTemplates npc = _perso.get_curCarte().getNPC(npcGUID).get_template();
	...
	public void commandGmFour(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("ITEMSET")) {
			...
			for (ObjTemplate t : IS.getItemTemplates()) {
				Objects obj = t.createNewItem(1, useMax, -1);
				if (perso != null) {
		...
		if (command.equalsIgnoreCase("ITEM")
			...
			if (qua < 1)
				qua = 1;
			Objects obj = t.createNewItem(qua, useMax, -1);
			...
			Objects obj = new Objects(World.getNewItemGuid(), tmorph.getID(), qua,
			...
			SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out, "Don de "
					+ regalo + " à " + objetivo.get_name());
		...
		if (command.equalsIgnoreCase("NEWITEM")) {
			...
			if (!donnate.equalsIgnoreCase("PA")
				...
				SocketManager
						.GAME_SEND_CONSOLE_ERROR_PACKET(_out,
		...
		if (command.equalsIgnoreCase("SPELLPOINT")) {
			...
			try{
				ResultSet RS = SQLManager.executeQuery("SELECT * FROM arena ORDER BY quote DESC LIMIT 0, 10", Config.DB_NAME);
				int rank = 0;
				while(RS.next()){
					rank++;
					if (rank < 5) {
						for (Characters c: Team.getPlayers(Team.getTeamByID(RS.getInt("id"))))
						{
							if (rank == 1) {
								...
								if (c.isOnline())
									SocketManager.GAME_SEND_MESSAGE(c, "Félicitation, vous venez de remporter 500 points pack avec la 1ere place du Top Pvp 2v2 !", AllColors.RED);
							} else if (rank == 2){
								int newPackPoints = Utils.loadPointsPackByAccount(c.get_compte()) + 400;
								Utils.updatePointsPackByAccount(c.get_compte(), newPackPoints);
								if (c.isOnline())
									SocketManager.GAME_SEND_MESSAGE(c, "Félicitation, vous venez de remporter 400 points pack avec la 2e place du Top Pvp 2v2 !", AllColors.RED);
							} else if (rank == 3){
								int newPackPoints = Utils.loadPointsPackByAccount(c.get_compte()) + 300;
								Utils.updatePointsPackByAccount(c.get_compte(), newPackPoints);
								if (c.isOnline())
									SocketManager.GAME_SEND_MESSAGE(c, "Félicitation, vous venez de remporter 300 points pack avec la 3e place du Top Pvp 2v2 !", AllColors.RED);
							} else if (rank == 4){
								int newPackPoints = Utils.loadPointsPackByAccount(c.get_compte()) + 200;
								Utils.updatePointsPackByAccount(c.get_compte(), newPackPoints);
								if (c.isOnline())
									SocketManager.GAME_SEND_MESSAGE(c, "Félicitation, vous venez de remporter 200 points pack avec la 4e place du Top Pvp 2v2 !", AllColors.RED);
							} else if (rank == 5){
								int newPackPoints = Utils.loadPointsPackByAccount(c.get_compte()) + 200;
								Utils.updatePointsPackByAccount(c.get_compte(), newPackPoints);
								if (c.isOnline())
									SocketManager.GAME_SEND_MESSAGE(c, "Félicitation, vous venez de remporter 200 points pack avec la 5e place du Top Pvp 2v2 !", AllColors.RED);
							}
						}
					}
				}
			...
			{
				team.getValue().setCote(0);
				Team.updateTeam(team.getKey());
			}
			SocketManager.GAME_SEND_MESSAGE_TO_ALL("Les récompenses d'arène 2v2 sont livrées ! La période d'arène est terminée, le classement retourne à 0. Bonne chance pour la nouvelle saison !!", AllColors.RED);
			return;
		}
