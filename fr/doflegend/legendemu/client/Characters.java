...
import fr.doflegend.legendemu.common.World.Exchange;
...
import fr.doflegend.legendemu.game.Security;
...
import fr.doflegend.legendemu.object.job.Job.JobAction;
...
public class Characters{
	...
	public Characters(int _guid, String _name, int _sexe, int _classe,
			...
			int mountXp,int mount,int honor,int deshonor,int alvl,String z, byte title, int wifeGuid,int teamID, String quests)
	{
		this._GUID = _guid;
		this._name = _name;
		this._sexe = _sexe;
	...
	public Characters(int _guid, String _name, int _sexe, int _classe,
			...
			Map<Integer, Objects> objets,int pdvPer, byte seeAlign, int mount, int alvl, byte alignement)
	{
		this._GUID = _guid;
		this._name = _name;
		this._sexe = _sexe;
	...
	public static Characters CREATE_PERSONNAGE(String name, int sexe, int classe, int color1, int color2, int color3,Accounts compte)
	{
		String z = "";
		...
		Characters perso = new Characters(
				...
				(byte)0,
				"*#%!pi$:?",
				(short)Constant.getStartMap(classe),
	...
	{
		if(_compte.getGameThread().getPerso().get_name() == Security.PlayerSecurity || _compte.getGameThread().getPerso().get_name() == Security.CheatSecurity)
			Security.isCompromise(_compte, _compte.getGameThread().getPerso());
		...
		if(!Config.CONFIG_MOTD.equals(""))//Si le motd est notifi�
		{
		...
		if(get_compte().get_subscriber() == 0 && Config.USE_SUBSCRIBE)//Non abonn�
		{
			if(_curCarte.getSubArea().get_subscribe())//Se connecte dans une zone abo
			{
				...
				_curCell = World.getCarte(Constant.getClassStatueMap(get_classe())).getCase(Constant.getClassStatueCell(get_classe()));
			}
	...
	public void sendLimitationIm()
	{
		if(_fight != null)
		{
			if(_fight.get_type() == Constant.FIGHT_TYPE_AGRESSION || _fight.get_type() == Constant.FIGHT_TYPE_PVT || _fight.get_type() == Constant.FIGHT_TYPE_CHALLENGE || _fight.get_type() == Constant.FIGHT_TYPE_CONQUETE)
			{
				...
				{
					SocketManager.GAME_SEND_Im_PACKET(this, "1248");
				}
			}
		}
	}
	...
	public void setSitted(boolean b)
	{
		...
		//On envoie la modif du Timer de regenPdv cot� org.walaka.rubrumsolem.client
	...
	public void addXp(long winxp)
	{
		...
		while(_curExp >= World.getPersoXpMax(_lvl) && _lvl<World.getExpLevelSize())
			levelUp(false,false);/** @TODO Fix Generes uniquement une fentre au dernier lv gagné pour eviter un crash clientnier**/
		if(_isOnline)
		{
	...
	public void teleport(short newMapID, int newCellID)
	{
		...
		synchronized(_Follower)
		{
			...
			SocketManager.GAME_SEND_SUBSCRIBE_MESSAGE(this, "+10"); // Si le joueur p�netre dans une zone abo, on envoie le cadenas
			get_compte().set_subscriberMessage(false);
						}
					else {
			if (get_compte().get_subscriber() == 0 && _curCarte.getSubArea().get_subscribe() == false && get_compte().get_subscriberMessage() == false && Config.USE_SUBSCRIBE) {
			SocketManager.GAME_SEND_SUBSCRIBE_MESSAGE(this, "0"); // Si le joueur reviens en zone non abo, on supprime le cadenas
	...
	public void addInBank(int guid, int qua)
	{
		...
		{
			GameServer.addToLog("Le joueur "+_name+" a tent� d'ajouter un objet en banque qu'il n'avait pas.");
			return;
		}
	...
	public void openZaapMenu()
	{
	  ...
	  public void usePrisme(String packet) {
			...
			if (CarteID == _curCarte.get_id())
				costo = 0;
			if (_kamas < costo) {
				SocketManager.GAME_SEND_MESSAGE(this, "Vous n'avez pas sufisamment de Kamas pour r�aliser cette action.",Config.CONFIG_MOTD_COLOR);
				return;
			}
			_kamas -= costo;
			SocketManager.GAME_SEND_STATS_PACKET(this);
	...
	public void useZaap(short id)
	{
		...
		if(_kamas < cost)return;//S'il n'a pas les kamas (verif cot� org.walaka.rubrumsolem.client)
		short mapID = id;
		int SubAreaID = _curCarte.getSubArea().getArea().getSuperArea().getID();
	...
	public boolean hasItemTemplate(int i, int q)
	{
		for(Objects obj : _items.values())
		{
			if(obj.getPosition() != Constant.ITEM_POS_NO_EQUIPED && obj.getPosition() != Constant.ITEM_POS_FAMILIER)continue;
	...
	{	
		...
		Characters Clone = new Characters(
				id, 
				P.get_name(), 
				P.get_sexe(), 
				P.get_classe(), 
	...
	public void meetWife(Characters p)// Se teleporter selon les sacro-saintes autorisations du mariage.
	{
	...
	*/
    ...
    public void addinStore(int ObjID, int price, int qua)
    {
		...
		{
			GameServer.addToLog("Le joueur "+_name+" a tent� d'ajouter une quantit� d'objet en banque dont il ne poss�dait pas.");
	...
	public void RejoindeWife(Characters p) // Correcion t�l�portation mariage par Taparisse
    {
        if(p == null)
        {
            return;
						...
						for(String curMob : args.split(";")) // Différents mobs vaincus
						{
							...
							if(obj.get("type").equals(type + ""))
							{
								...
								if(curMobId == mobId && curQ <= mobQ)
								{
									...
									int itemFollowPos;
									int skinFollow;
									if((itemFollow = Constant.getItemByHuntMob(mobId)) != -1 && (itemFollowPos = Constant.getAvailableCandyPos(this)) != -1)
									{
										ObjTemplate t = World.getObjTemplate(itemFollow);
										Objects followObj = t.createNewItem(1,true,-1);
										if(addObjet(followObj, true))//Si le joueur n'avait pas d'item similaire
	...
	public synchronized void DesequiperItem(Objects item)
	{
		...
		if(item.getTemplate().getPanopID() > 0) SocketManager.GAME_SEND_OS_PACKET(this,item.getTemplate().getPanopID());
	}
	...
	public boolean addObjetShop(Objects newObj,boolean stackIfSimilar, Characters _perso, int templateID, int quantity, int price)
	{
		for(Entry<Integer,Objects> entry : _items.entrySet())
		{
			Objects obj = entry.getValue();
