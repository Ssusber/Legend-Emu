...
import fr.doflegend.legendemu.check.FloodCheck;
...
import fr.doflegend.legendemu.command.GmCommand;
import fr.doflegend.legendemu.command.PlayerCommand;
...
import fr.doflegend.legendemu.game.tools.ParseTools;
...
public class GameThread implements Runnable { 
	...
	public GameThread(Socket sock) {
		try {
			...
			_t = new Thread(LegendEmu.THREAD_GAME, this);
			_t.setDaemon(true);
			try {
				_t.start();
	...
	public void run() {
		try {
	    	...
	    	while(result!=-1 && LegendEmu.isRunning) {
	...
	private void ParseConquetePacket(String packet) {
		switch (packet.charAt(1)) {
			...
			case 'b':
				SocketManager.SEND_Cb_CONQUETE(_perso, World.getBalanceMundo(_perso.get_align()) + ";"
						+ World.getBalanceArea(_perso.get_curCarte().getSubArea().getArea(), _perso.get_align()));
				break;
			case 'B':
	...
	private void geoConquest(String packet) {
		switch (packet.charAt(2)) {
			case 'J':
				SocketManager.SEND_CW_INFO_CONQUETE(_perso, World.PrismesGeoposition(_perso.get_align()));
				break;
			case 'V':
				SocketManager.SEND_CIV_INFOS_CONQUETE(_perso);
				break;
		...
		switch (packet.charAt(2)) {
			case 'J':
				...
				if (_perso.get_fight() != null)
					return;
				if (_perso.get_curCarte().get_id() != CarteID) {//System.out.println("Fuck5");
	...
	private void parseGuildPacket(String packet)
	{
		...
		private void guild_perco_join_fight(String packet) 
		{
			...
			case 'J'://Rejoindre
				...
				if(Config.debug) GameServer.addToLog("[DEBUG] Percepteur INFORMATIONS : TiD:"+TiD+", FightID:"+FightID+", MapID:"+MapID+", CellID"+CellID);
	...
	private void guild_remove_perco(String packet) 
	{
		...
		if(!_perso.getGuildMember().canDo(Constant.G_POSPERCO))return;//On peut le retirer si on a le droit de le poser
	...
	{
		if(_perso.get_guild() == null || _perso.get_fight() != null || _perso.is_away())return;
		if(!_perso.getGuildMember().canDo(Constant.G_POSPERCO))return;//Pas le droit de le poser
		...
		if(Collector.CountPercoGuild(_perso.get_guild().get_id()) >= _perso.get_guild().get_nbrPerco()) return;//Limite de percepteur
	...
	private void guild_kick(String name)
	{
		...
		if(!_perso.getGuildMember().canDo(Constant.G_BAN) && _perso.getGuildMember().getGuid() != toRemMember.getGuid())
		{
			SocketManager.GAME_SEND_gK_PACKET(_perso, "Ed");
			return;
		}
	...
	private void guild_join(String packet)
	{
		...
		{
		case 'R'://Nom perso
			Characters P = World.getPersoByName(packet.substring(1));
	...
	private void parseObjectPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			...
			case 'M'://Bouger un objet (Equiper/déséquiper) // Associer obvijevan
				...
				Object_move(_perso, _out, qua, guid, pos);
				break;
	...
	public static synchronized void Object_move(Characters _perso, GameSendThread _out, int qua, int guid, int pos)
	{
		try
		{
			Objects obj = World.getObjet(guid);
	...
	private void forgetSpell(String packet)
	{
		...
		if(Config.debug) GameServer.addToLog("Info: "+_perso.get_name()+": Tente Oublie sort id="+id);
		
		if(_perso.forgetSpell(id))
		{
			if(Config.debug) GameServer.addToLog("Info: "+_perso.get_name()+": OK pour Oublie sort id="+id);
			SocketManager.GAME_SEND_SPELL_UPGRADE_SUCCED(_out, id, _perso.getSortStatBySortIfHas(id).getLevel());
			SocketManager.GAME_SEND_STATS_PACKET(_perso);
				...
				case 'M' :
					try {
						if (_perso.get_compte().get_vip() >= 1 || _perso.get_compte().get_gmLvl() > 0){
							...
							String posComplete = packet.substring(3).trim() + "," + _perso.get_curCarte().getSubArea().getArea().getSuperArea().getID();
	...
	{
		if(command == null) command = new GmCommand(_perso);
	...
	private void Basic_chatMessage(String packet)
	{
		...
		switch(packet.charAt(2))
		{
			...
			case '^':// Canal Incarnam 
				...
				{
					x = (Config.FLOOD_TIME - x)/1000;//Calculamos a diferença em segundos 
	...
	{	
		...
		int a = 0;
		if (GA._actionID == 1) a = 1;
		removeAction(GA);
		if (wait != 0) wait = 0;
		if ((a == 1) && (GA_wait != null)) parseGameActionPacket(GA_wait._packet);
		}
	...
	private void Game_on_GI_packet()
	  {
	    ...
	    try
	    {
	      ...
	      SocketManager.GAME_SEND_Rp_PACKET(this._perso, this._perso.get_curCarte().getMountPark()); debug++;

	      Houses.LoadHouse(this._perso, this._perso.get_curCarte().get_id()); debug++;

	      SocketManager.GAME_SEND_MAP_GMS_PACKETS(this._perso.get_curCarte(), this._perso); debug++;
	      SocketManager.GAME_SEND_MAP_MOBS_GMS_PACKETS(this._perso.get_compte().getGameThread().get_out(), this._perso.get_curCarte()); debug++;
	      SocketManager.GAME_SEND_MAP_NPCS_GMS_PACKETS(this._out, this._perso.get_curCarte()); debug++;
	      SocketManager.GAME_SEND_MAP_PERCO_GMS_PACKETS(this._out, this._perso.get_curCarte()); debug++;

	      SocketManager.GAME_SEND_MAP_OBJECTS_GDS_PACKETS(this._out, this._perso.get_curCarte()); debug++;
	      SocketManager.GAME_SEND_GDK_PACKET(this._out); debug++;
	      SocketManager.GAME_SEND_MAP_FIGHT_COUNT(this._out, this._perso.get_curCarte()); debug++;
		  SocketManager.GAME_SEND_MERCHANT_LIST(_perso, _perso.get_curCarte().get_id()); debug++;
		  SocketManager.SEND_GM_PRISME_TO_MAP(this._out, map); debug++;
	      Fight.FightStateAddFlag(this._perso.get_curCarte(), this._perso); debug++;

	      this._perso.get_curCarte().sendFloorItems(this._perso); debug++;
	    }
	    catch (Exception e)
	    {
	...
	/**private void Game_on_GI_packet() 
	{
		if(_perso.get_fight() != null)
		{
			...
			//Only percepteur
			if(_perso.get_fight().get_type() == Constants.FIGHT_TYPE_PVT) SocketManager.GAME_SEND_MAP_GMS_PACKETS(_perso.get_fight().get_map(), _perso);
			SocketManager.GAME_SEND_GDK_PACKET(_out);
			return;
		}
		//Enclos
	...
	private void Game_attaque_prisme(String packet) {
		try {
			...
			_perso.get_curCarte().iniciarFightVSPrisme(_perso, Prisme);
			...
			try
			{
				...
				if((World.getPersonnage(guid).get_fight().get_type() == Constant.FIGHT_TYPE_AGRESSION || 
						World.getPersonnage(guid).get_fight().get_type() == Constant.FIGHT_TYPE_CONQUETE || 
								World.getPersonnage(guid).get_fight().get_type() == Constant.FIGHT_TYPE_PVT) && _perso.get_compte().get_subscriber() == 0 && Config.USE_SUBSCRIBE)
					{
					SocketManager.GAME_SEND_SUBSCRIBE_MESSAGE(_perso, "+5");
						return;
					}
	...
	public void kick()
	{
		try
		{
			...
			} catch (Exception e) {System.out.println("Une erreur vient de se produire lors de la déconnexion du compteID:"+_compte.get_GUID());}
			
		
				LegendEmu.gameServer.delClient(this);
	...
	private void parseAccountPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			...
			case 'T':
				...
				_compte = LegendEmu.gameServer.getWaitingCompte(guid);
				if(_compte != null)
				{
					...
					_compte.setGameThread(this);
					_compte.setCurIP(ip);
					LegendEmu.gameServer.delWaitingCompte(_compte);
