...
public class AccountQueue {
	...
	public static void verify(Client client) {
		...
		if(worldAccount.isOnline() && worldAccount.getGameThread() != null) state = 2;
		if(worldAccount.getGameThread() != null) state = 2;
		if(worldAccount.isBanned()) state = 3;
		//if(World.isIpBanned(ip, worldAccount)) state = 3;
		if(!Config.CONFIG_ALLOW_MULTI && World.ipIsUsed(ip)) state = 4;
		if(World.getGmAccess() > worldAccount.get_gmLvl()) state = 4;
