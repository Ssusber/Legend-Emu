...
public class World {
	...
	public static Map<Integer,StatsMetier> upFPc = new TreeMap<Integer,StatsMetier>(); // P�cheurman... 42.. Sydney... avenue Walaby
	...
	public static class ItemSet
	{
	 ...
	 public static class SubArea {
			...
			public SubArea(int id, int areaID, int alignement, String nom, int conquistable, int Prisme, boolean subscribe) {
				_id = id;
				_nom = nom;
	...
	public static void createWorld() {
		...
		Console.bright();
		Console.println("\n			Load the dynamic datas : ", Color.YELLOW);
	...
	{
		return (getComptebyName().get(name.toLowerCase())!=null?getComptes().get(getComptebyName().get(name.toLowerCase())):null);
	}
	...
	public static Characters getPersoByName(String name)
	{
		...
		for(Characters P : Ps)if(P.get_name().equalsIgnoreCase(name))return P;
		return null;
	}
	...
	public static int countPersoOnMap(short mapid)
	{
		...
		if(map.getPersos() == null)return 0;
	...
	public static List<Characters> getOnlinePersos()
	{
		...
		{
			if(perso.getValue() != null && perso.getValue().isOnline() && perso.getValue().get_compte()!=null && perso.getValue().get_compte().getGameThread() != null)
			{
		...
		}catch(ConcurrentModificationException e)
		{
			if(saveTry < 10)
			{
				...
				if(saver != null && _out != null)
					SocketManager.GAME_SEND_CONSOLE_SUCCESS_PACKET(_out, "Erreur. Nouvelle tentative de sauvegarde");
				saveTry++;
				saveAll(saver);
			}
			else
	...
	{
		SocketManager.GAME_SEND_MESSAGE_TO_ALL("Recharge des Mobs en cours, des latences peuvent survenir.", Config.CONFIG_MOTD_COLOR);
		for(Maps map : getCartes().values())
		{
			map.refreshSpawns();
		}
		SocketManager.GAME_SEND_MESSAGE_TO_ALL("Recharge des Mobs finie. La prochaine recharge aura lieu dans 5heures.", Config.CONFIG_MOTD_COLOR);
	}
	...
	{
		for(Accounts C : getComptes().values())if(C.get_pseudo().equals(p))return C;
		return null;
	}
	...
	{
		int max = 1;
		for(int a : getDragodindes().keySet())if(a > max)max = a;
		return max+1;
	}
	...
	public synchronized static int getNextIDPrisme() {
		int max = 1;
		for(int a : Prismes.keySet())if(a > max)max = a;
		return max+1;
	}
  public synchronized static void addPrisme(Prism Prisme) {
	...
	public static String PrismesGeoposition(int alignement) {
		...
		for (SubArea subarea : getSubAreas().values()) {
			...
			if (first)
				str += ";";
			str += subarea.getID() + "," + (subarea.getalignement() == 0 ? -1 : subarea.getalignement()) + ",0,";
	...
	{
		if(getGuildes().isEmpty())return 1;
		int n = 0;
		for(int x : getGuildes().keySet())if(n<x)n = x;
		return n+1;
	}

	public static boolean guildNameIsUsed(String name)
	{
		for(Guild g : getGuildes().values())if(g.get_name().equalsIgnoreCase(name))return true;
		return false;
	}
	...
	{
		if(World.getGuild(id) == null) return;
	...
	{
		for(Accounts c : getComptes().values())if(c.get_curIP().compareTo(ip) == 0)return true;
		return false;
	}
	...
	public static void addHdvItem(int compteID, int hdvID, HdvEntry toAdd)
	{
		if(get_hdvsItems().get(compteID) == null)	//Si le compte n'est pas dans la memoire
			get_hdvsItems().put(compteID,new HashMap<Integer,ArrayList<HdvEntry>>());	//Ajout du compte cl�:compteID et un nouveau map<hdvID,items<>>
	...
	public static boolean mariageok(){ // Le mariage est-il ok ?
		...
		boolean b = false;
		try{
			if(getMarried().get(1) != null) a = true;
			if(getMarried().get(2) != null) b = true;
		}catch(Exception e){
	...
	public static void PriestRequest(Characters perso, Maps carte, int IdPretre)
	{
		...
		if(Homme.getWife() != 0){
			SocketManager.GAME_SEND_MESSAGE_TO_MAP(carte, Homme.get_name()+" est deja marier!", Config.CONFIG_MOTD_COLOR);
			return;
		}
		if(Femme.getWife() != 0){
	...
	public static void Banip(Accounts c, int nb_heures)
	{
		...
		else nb_heures = nb_heures*3600;
		String ip = c.get_curIP();
		if(ip.equals("")) ip = c.get_lastIP();
		if(ip.equals("")) return;
		SQLManager.LOAD_ACCOUNT_BY_IP(ip);
		BanIp ban;
		if(nb_heures == -1) ban = new BanIp(ip, nb_heures);
		else ban = new BanIp(ip, (long) (System.currentTimeMillis()/1000 + nb_heures));
		getBanips().add(ban);
		...
		for(Accounts compte : getComptes().values())
		{
			if((compte.get_lastIP().equals(ip) || compte.get_curIP().equals(ip)) && compte.get_gmLvl() < 4)
			{
				...
				if(compte.getGameThread() != null) compte.getGameThread().kick();
			}
		}
	}
	...
	public static void removeBanip(String ip)
	{
		...
		for(BanIp ban : getBanips())
		{
			...
			if(ban.getIp().equalsIgnoreCase(ip)) continue;
			bans.add(ban);
		}
		getBanips().clear();
	...
	public static boolean isIpBanned(String ip, Accounts c)
	{
		...
		for(BanIp ban : getBanips())
		{
			...
			{
				if(!c.isBanned()) c.ban(ban.getTime(), true);
				isIpBanned = true;
			}
			bans.add(ban);
		}
	...
	public static class BanIp
	{
		...
		{
			if(_time == -1) return true;
			if(_time < System.currentTimeMillis()/1000) return false;
			return true;
		}
	}
